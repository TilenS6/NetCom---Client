...
int NetClient::recieveData() { // TODO non blocking
    int iResult = recv(ConnectSocket, recvbuf, recvbuflen, 0);

    if (iResult == -1 && WSAGetLastError() == WSAEWOULDBLOCK) return recieveData_NO_NEW_DATA;

    if (iResult > 0) return recieveData_OK;
